...
$( document ).ready( () => { 
    ...
    $("#countdown").click( () => {
        ...
        // defining two constants month and day for putting validations and getting the value of zero index of array for month and 
        // value at index 1 for days
        const month = parseInt(dateParts[0]);
        const day = parseInt(dateParts[1]);

        if (month < 1 || month > 12) {
         messageLbl.text("Please enter the value of month between 1 and 12.");
        return;
        }

        const validDay = ValidDayOfMonth(month, day, year);
        if (!validDay) {
        messageLbl.text("The day for this month is invalid.");
        return;
        }

        //creatung a function which defines the maximum number of days in each month
        function ValidDayOfMonth(month, day, year) {
        ...
        return day >= 1 && day <= maxDays.get(month) && day >= 0;
        }

        //creatung a function for leap year
